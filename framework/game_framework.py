...
class Agent:
    ...
    def get_action(self, response):
        ...
        action = self.actions.get_action(invocation["tool"].lower())
        return action, invocation

    def should_terminate(self, response: str) -> bool:
        action_def, _ = self.get_action(response)
        if action_def is None:          # ← guard
            return False
    ...
    def update_memory(self, memory: Memory, response: str, result: dict):
        ...
        new_memories = [
            ...
            {"type": "environment", "content": result_str}
            ]
    ...
    def run(self, user_input: str, memory=None, max_iterations: int = 50) -> Memory:
        ...
        for _ in range(max_iterations):
            ...
            if action is None:
                # El LLM llamó a una tool que no existe, registramos el error y continuamos
                memory.add_memory({
                    "type": "system",
                    "content": f"Tool '{invocation.get('tool')}' not found. Use only the available tools."
                    })
                continue
